...
if __name__ == '__main__':
    ...
    NORMALIZATION = True
    DUAL = True
    x_sampler= 'npuniform' # 'uniform, npuniform,shuffle'
    ...
    for i in range(n_rep):
      ...
      start = timer()
      gan.train(x, batch_size=batch_size, steps=steps, save_samples_every=25000, log_losses=False,logdir='out_CIFAR10_final')
      end = timer()
      time = (end - start)
      ...
      gan.save_generator('out_CIFAR10', name= GANname + '_' + str(i))
      tf.keras.backend.clear_session()  # free memory
      del gan
  
    # Print results 
    for k,v in cifar_results_i.items():       
      write_to_console_file('\n FINAL RESULTS CIFAR10' + k)
      arr = np.asarray(v)
