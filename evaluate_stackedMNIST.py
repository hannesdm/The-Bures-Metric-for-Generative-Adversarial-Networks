...
if __name__ == '__main__':
    ...
    x_sampler_args={'sample_cache_nr':'all'} # precalculate an amount of batches - for LS this means only calculate leverage score every x batches. Set to None or a very high number when using uniform sampling
    ...
    if GANname == 'AlternatingBures':
        def create_gan_model():
            return AlternatingBuresGAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                     models.mnist_dcgan_discriminator_model(channels=channels,n_layers = n_layers, return_feature_map=True),x_sampler_args=x_sampler_args,name=None ,diversity_loss_func=div_func)  
    elif GANname == 'Bures':
        def create_gan_model():
            return BuresGAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
        ...
        def create_gan_model():
            return GAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                models.mnist_dcgan_discriminator_model(channels=channels,n_layers = n_layers),x_sampler_args=x_sampler_args)
    elif GANname == 'WGANGP':
        def create_gan_model():
            return WGAN_GP(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                    models.mnist_dcgan_discriminator_model(channels=channels,n_layers = n_layers,batchnorm=False),
                        x_sampler_args=x_sampler_args, lam=10.0)
    elif GANname == 'VEEGAN':
        def create_gan_model():
            return VEEGAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
        ...
        def create_gan_model():
            return MDGANv1(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                  models.mnist_dcgan_discriminator_model(channels=channels,n_layers = n_layers),
                  encoder=models.mnist_dcgan_encoder_model(z_dim=z_dim, channels=channels),x_sampler_args=x_sampler_args)
    elif GANname == 'MDGANv2':
        def create_gan_model():
            return MDGANv2(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                  models.mnist_dcgan_discriminator_model(channels=channels,n_layers = n_layers),
                  encoder=models.mnist_dcgan_encoder_model(z_dim=z_dim, channels=channels),x_sampler_args=x_sampler_args)
    elif GANname == 'UnrolledGAN':
        def create_gan_model():
            return UnrolledGAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                        models.mnist_dcgan_discriminator_model(batchnorm=False,n_layers = n_layers,channels=channels),x_sampler_args=x_sampler_args)
    elif GANname == 'GDPPGAN':
        def create_gan_model():
            return GDPPGAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                    models.mnist_dcgan_discriminator_model(channels=channels,n_layers = n_layers, return_feature_map=True),x_sampler_args=x_sampler_args)
    elif GANname == 'PACGAN':
        def create_gan_model():
            return PACGAN(models.mnist_dcgan_generator_model(z_dim=z_dim, channels=channels),
                          models.mnist_dcgan_discriminator_model(channels=channels * 2, n_layers=n_layers),
                          x_sampler_args=x_sampler_args, pack_nr=2, name='PACGAN2')
    elif GANname == 'frobenius':
        def create_gan_model():
            return DiverseGAN(models.mnist_dcgan_generator_model_original_veegan(z_dim=z_dim, channels=channels),
                            models.mnist_dcgan_discriminator_model(channels=channels, n_layers=n_layers,
